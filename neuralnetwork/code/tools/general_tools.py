...
def run_subprocess(program):
	...
	while not returnCode==0:
		p = subprocess.Popen(program, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
		returnCode = None
...
def make_video(data,dt,fileName,windowLength):
	...
	gs = gridspec.GridSpec(len(list(data.keys())),1)
	...
	startInd = 0
	endInd = len(time)
	nSamplesToShift = int(old_div(1000./fps,dt))
